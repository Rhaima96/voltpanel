...
const hasZiggy = (): boolean => {
  return typeof window !== 'undefined' && typeof (window as any).route === 'function';
};

// Check if Wayfinder routes are available
const hasWayfinder = (): boolean => {
 ...
 */
export const getRoutingMode = (): 'ziggy' | 'wayfinder' | 'fallback' => {
 ...
 *
 * @param name - The route name (e.g., 'voltpanel.resources.index')
 * @param params - Route parameters
 * @returns The generated URL string
 */
export const generateRoute = (name: string, params: RouteParams = {}): string => {
  // Try Ziggy first
  if (hasZiggy()) {
    try {
 ...
 */
const buildFallbackUrl = (name: string, params: RouteParams): string => {
  const basePath = getBasePath();

  // Map route names to URL patterns
  const routePatterns: Record<string, (params: RouteParams) => string> = {
    // VoltPanel routes
...
export function useRouting() {
  ...
  const currentRoute = (): string | null => {
    if (hasZiggy()) {
      try {
        return (window as any).route().current() || null;
      } catch {
        return null;
      }
    }
   ...
   */
  const isCurrentRoute = (name: string, params?: RouteParams): boolean => {
    if (hasZiggy()) {
      try {
        return (window as any).route().current(name, params);
      } catch {
        return false;
      }
    }
